# CAP776 Python Project - Student Daily Activity Evaluation
...
# Reading row 5 to dynamically identify column index positions
...
# Calculating expected days between 13-08-2026 and 21-09-2026
        date_start = dt.datetime(2026, 8, 13)
        date_end = dt.datetime(2026, 9, 21)
        self.expected_days_count = (date_end - date_start).days + 1

# Checking total tracked time > 0 to identify valid tracking days
...
# 1. Tech Productivity Index (Daily coding average)
    def get_tpi(self):
...
# 2. Academic Activity Index (Study + Class daily commitment)
    def get_aai(self):
...
# 3. Physical Health Activity Index (Fitness compliance)
    def get_phai(self):
...
# 4. Sleep Regularity Index (Daily sleep hours)
    def get_sri(self):
...
# 5. Active Balance Index (Downtime / free unaccounted time)
    def get_abi(self):
...
# 6. Time Utility Index (Total logged hours per day)
    def get_tui(self):
...
# 7. Emotional Index (Psychological score normalized to 5.0)
    def get_ei(self):
...
# 8. Data Continuity Index (valid days / expected days * 100)
    def get_dci(self):
...
# Composite PAI weighted summation (Rubric breakdown)
    composite_pai = (
        (0.15 * tpi_res) +
        (0.20 * aai_res) +
        (0.15 * phai_res) +
